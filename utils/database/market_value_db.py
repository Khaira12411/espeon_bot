...
async def load_market_cache_from_db(bot) -> dict:
    ...
    try:
        ...
        market_value_cache.update(cache)  # Update the global cache first
        # Build Pokemon list cache from market value cache
        build_pokemon_list_from_cache()
        return market_value_cache
...
def build_pokemon_list_from_cache():
    ...
    using the global market_value_cache.
    """
    if not market_value_cache:
        espeon_log(
            tag="cache",
            message="⚠️ Pokémon cache is empty, cannot build list."
        )
        return {}
    ...
    espeon_log(
        tag="cache",
        message=f"✅ Built Pokémon list with {len(pokemon_list_cache)} entries."
    )
