...
from utils.database.server_shop import fetch_item_by_id, remove_item, remove_all_items
...
async def remove_item_func(
    ...
    embed = discord.Embed(
        ...
        description=f"**Item ID:** `{item_id}`\n**Item Name:** {existing_item['item_name']}",
        color=COLOR,
...
async def shop_clear_func(
    ...
    # Defer
    loader = await pretty_defer(
        interaction=interaction, content="Clearing all items from shop...", ephemeral=True
    )
